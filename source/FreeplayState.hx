...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		textBG.alpha = 0.6;
		add(textBG);
		var text:FlxText = new FlxText(textBG.x, textBG.y + 4, FlxG.width, "Press X to play the song / Press Y to open the Gameplay Changers Menu.", 18);
	...
	override function update(elapsed:Float)
	{
		...
		var space = FlxG.keys.justPressed.SHIFT #if mobile || virtualPad.buttonY.justPressed #end;
		var playso = FlxG.keys.justPressed.SPACE #if mobile || virtualPad.buttonX.justPressed #end;
		...
		else if (accepted)
		{
			...
			#if MODS_ALLOWED
			if(!Util.exists(Paths.modsJson(songLowercase + '/' + poop)) && !Util.exists(Paths.json(songLowercase + '/' + poop))) {
			#else
